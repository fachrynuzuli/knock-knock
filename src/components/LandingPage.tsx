...
const LandingPage: React.FC<LandingPageProps> = ({ onEnterGameFlow }) => {
  ...
  const [showScrollInstruction, setShowScrollInstruction] = useState(false);
  ...
  useEffect(() => {
    ...
    document.body.style.overflow = 'hidden'; // Start with scroll locked
  ...
  // Monitor scroll position to detect when user returns to top
  useEffect(() => {
    if (!containerRef.current || !scrollEnabled) return;

    const handleScroll = () => {
      if (containerRef.current) {
        const scrollTop = containerRef.current.scrollTop;
        const newIsAtTop = scrollTop < 50; // Small threshold for "at top"
        
        if (newIsAtTop !== isAtTop) {
          setIsAtTop(newIsAtTop);
          
          // If user scrolled back to top, re-enable scroll lock
          if (newIsAtTop) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 relative">
      ...
      >
        ...
        <motion.div 
          className="fixed inset-0 overflow-hidden pointer-events-none scroll-optimized" 
        ...
        <motion.div 
          className="fixed inset-0 pointer-events-none z-10 scanlines-effect scroll-optimized" 
          style={{ y: scanlinesY }}
          aria-hidden="true" 
        />
        ...
        <div className="relative z-20 flex flex-col">
          {/* Hero Section - LOCKED UNTIL ARROW CLICKED */}
          <motion.section 
            className="min-h-screen flex items-center justify-center px-4 py-12 parallax-section"
            style={{ 
              y: scrollEnabled ? heroY : 0, 
              scale: scrollEnabled ? heroScale : 1, 
              opacity: scrollEnabled ? heroOpacity : 1 
            }}
          >
            <div className="max-w-6xl mx-auto text-center">
              ...
              <motion.div
                ...
                style={{ 
                  y: scrollEnabled ? characterY : 0, 
                  scale: scrollEnabled ? characterScale : 1, 
                  rotate: scrollEnabled ? characterRotate : 0 
                }}
                className="mb-8"
              >
              ...
              <motion.h1
                ...
                style={{ 
                  scale: scrollEnabled ? titleScale : 1, 
                  y: scrollEnabled ? titleY : 0 
                }}
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={!scrollEnabled ? { 
                    scale: 1.1,
                    y: -5,
                    transition: { duration: 0.2 }
                  } : {}}
                  whileTap={!scrollEnabled ? { 
                    scale: 0.95,
                    transition: { duration: 0.1 }
                  } : {}}
                  animate={!scrollEnabled ? {
                    y: [0, -8, 0],
                    scale: [1, 1.05, 1],
                  } : {}}
                  transition={!scrollEnabled ? {
                    duration: 2,
                    repeat: Infinity,
                    ease: [0.4, 0, 0.6, 1]
                  } : {}}
                  className={`group relative bg-gradient-to-b from-primary-500 to-primary-700 hover:from-primary-400 hover:to-primary-600 text-white p-4 rounded-full shadow-2xl transition-all duration-300 ${
                    scrollEnabled ? 'opacity-50 cursor-not-allowed' : 'glow-border cursor-pointer'
                  }`}
                >
                  {/* Pulsing ring effect */}
                  {!scrollEnabled && (
                  ...
                  >
                    {scrollEnabled ? 'Scroll unlocked!' : 'Click to unlock scrolling'}
                  </motion.div>
                </motion.button>
              </motion.div>
            </div>
          ...
          {/* LIVE PLAYABLE GAME DEMO SECTION - Enhanced with parallax when unlocked */}
          <motion.section 
            className="py-20 px-4 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 parallax-section"
            style={{ 
              y: scrollEnabled ? videoY : 0, 
              scale: scrollEnabled ? videoScale : 1, 
              opacity: scrollEnabled ? videoOpacity : 1 
            }}
          >
          ...
          {/* Features Section with enhanced parallax when unlocked */}
          <motion.section 
            className="py-16 px-4 parallax-section"
            style={{ 
              y: scrollEnabled ? featuresY : 0, 
              opacity: scrollEnabled ? featuresOpacity : 1 
            }}
          >
            <div className="max-w-6xl mx-auto">
              <motion.h2
                ...
                {[
                  {
                    ...
                    description: "See what everyone's working on in a visual, engaging neighborhood format. No more wondering what your teammates accomplished this week.",
                    color: "primary"
                  },
                  {
                    icon: Trophy,
                    title: "Gamified Progress",
                    description: "Earn badges, level up your house, and celebrate achievements. Turn routine reporting into an engaging experience your team will love.",
                    color: "secondary"
                  },
                  {
                    icon: BarChart3,
                    title: "Smart Analytics",
                    description: "Get insights into team productivity, project progress, and individual contributions through beautiful, actionable analytics.",
                    color: "warning"
                  }
                ].map((feature, index) => (
                  <motion.div
                  ...
                  >
                    <motion.div 
                      className={`bg-${feature.color}-600 p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center`}
                      whileHover={{ 
                        rotate: 360,
                    ...
                    <h3 className={`text-xl font-heading text-${feature.color}-400 mb-3`}>{feature.title}</h3>
          ...
          {/* How It Works Section with enhanced staggered parallax when unlocked */}
          <motion.section 
            className="py-16 px-4 bg-gray-800 bg-opacity-30 parallax-section"
            style={{ 
              y: scrollEnabled ? howItWorksY : 0, 
              scale: scrollEnabled ? howItWorksScale : 1 
            }}
          >
            <div className="max-w-6xl mx-auto">
              <motion.h2
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { number: 1, title: "Create Neighborhood", description: "Set up your team's virtual neighborhood and invite members", color: "primary" },
                  { number: 2, title: "Log Activities", description: "Update your activity board with weekly accomplishments", color: "secondary" },
                  { number: 3, title: "Explore & Engage", description: "Visit teammates' houses and celebrate their achievements", color: "warning" },
                  { number: 4, title: "Level Up", description: "Earn badges and upgrade your house as you contribute", color: "success" }
                ].map((step, index) => (
                  <motion.div
                    key={index}
                  ...
                  >
                    <motion.div 
                      className={`bg-${step.color}-600 text-white w-12 h-12 rounded-full flex items-center justify-center font-heading text-lg mx-auto mb-4`}
                      whileHover={{ 
                        rotate: 360, 
                        scale: 1.2,
                    ...
                    <h3 className={`text-lg font-heading text-${step.color}-400 mb-2`}>{step.title}</h3>
                    <p className="text-gray-300 font-pixel text-sm">
                      {step.description}
                    </p>
                  </motion.div>
          ...
          {/* Final CTA Section with dramatic enhanced parallax when unlocked */}
          <motion.section 
            className="py-16 px-4 parallax-section"
            style={{ 
              y: scrollEnabled ? finalCtaY : 0, 
              scale: scrollEnabled ? finalCtaScale : 1 
            }}
          >
